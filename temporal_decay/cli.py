"""
Command-Line Interface for TemporalDecay: Time-Weighted Memory Salience & Exponential Forgetting Agent.
"""
import argparse
import csv
import json
...
from .agents import TemporalDecayCoordinator

coordinator = TemporalDecayCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="temporal-context-time-decay-agent", description="TemporalDecay: Time-Weighted Memory Salience & Exponential Forgetting Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  TEMPORALDECAY: TIME-WEIGHTED MEMORY SALIENCE & EXPONENTIAL FORGETTING AGENT")
        print(f"  Domain: Autonomous Context Management & State Engines | Standard: Cognitive Psychology Ebbinghaus Decay Models")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting TemporalDecay: Time-Weighted Memory Salience & Exponential Forgetting Agent on http://{args.host}:{args.port}")
